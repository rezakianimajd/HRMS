...
import {
  ...
  TextField, Alert, Switch, FormControlLabel,
} from '@mui/material';
...
const PettyCashPolicyPage = () => {
  const qc = useQueryClient();
  const [form, setForm] = useState({ name: '', single_level_limit: '0' });
  const [editing, setEditing] = useState(null);

  const { data, isLoading } = useQuery({ queryKey: ['petty-policies'], queryFn: () => axiosInstance.get('/petty-cash-approval-policies/').then(r => r.data) });
  const list = Array.isArray(data) ? data : data?.results || [];

  const save = useMutation({
    mutationFn: (p) => editing ? axiosInstance.patch(`/petty-cash-approval-policies/${editing.id}/`, p) : axiosInstance.post('/petty-cash-approval-policies/', p),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['petty-policies'] }); setEditing(null); setForm({ name: '', single_level_limit: '0' }); },
  });
    ...
    <Box>
      <Paper sx={{ p: 2.5, mb: 2.5, display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap',
        ...
        <Avatar sx={{ width: 56, height: 56, background: `linear-gradient(135deg,${COLOR},${COLOR_DARK})`, boxShadow: `0 8px 24px ${COLOR}55` }}>
          <GavelIcon sx={{ fontSize: 28, color: '#fff' }} />
        </Avatar>
        <Box sx={{ flex: 1 }}>
          <Typography variant="h6" fontWeight={800} color={COLOR_DARK}>سیاست تأیید تنخواه</Typography>
        ...
        <Stack spacing={1}>
          {list.map(p => (
            <Paper key={p.id} sx={{ ...glass, p: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Avatar sx={{ bgcolor: COLOR }}><GavelIcon /></Avatar>
              <Box sx={{ flex: 1 }}>
...
export default PettyCashPolicyPage;