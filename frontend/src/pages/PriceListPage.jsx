    ...
    <Box>
      <Paper sx={{ p: 2.5, mb: 2.5, display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap',
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={openPLDialog}
          sx={{ background: 'linear-gradient(135deg,#f59e0b,#3b82f6)', borderRadius: 10 }}>
          فهرست‌بها جدید
        </Button>
      </Paper>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ ...glassPaper, p: 2 }}>
            ...
            <Stack spacing={0.75}>
              {priceLists.map(pl => (
                <Paper key={pl.id} variant="outlined" sx={{ p: 1, borderRadius: 10, cursor: 'pointer',
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ ...glassPaper, p: 2 }}>
            ...
            {!selectedPriceList ? (
              ...
              <Stack spacing={0.75}>
                {versions.map(v => (
                  <Paper key={v.id} variant="outlined" sx={{ p: 1, borderRadius: 10, cursor: 'pointer',
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ ...glassPaper, p: 2 }}>
            ...
            {!selectedVersion ? (
              ...
              <Stack spacing={0.75}>
                {chapters.map(ch => (
                  <Paper key={ch.id} variant="outlined" sx={{ p: 1, borderRadius: 10, background: 'rgba(255,255,255,0.5)' }}>
