    ...
    <Box>
      ...
      <Paper sx={{
        ...
        <Button variant="contained" startIcon={<PersonAddIcon />} onClick={() => setOpenReq(true)}
          sx={{ background: 'linear-gradient(135deg, #0ea5e9, #8b5cf6)', borderRadius: 2, px: 2.5 }}>
            ...
            {STAGE_FLOW.map(stage => {
              ...
              return (
                <Grid item xs={12} sm={6} md={4} lg={2.4} key={stage}>
                  <Paper sx={{ p: 1.25, borderRadius: 2.5, minHeight: 200, background: 'rgba(255,255,255,0.55)' }}>
                    <Typography variant="caption" fontWeight={800} sx={{ color: STAGE_COLORS[stage], mb: 1, display: 'block' }}>
                      {STAGE_LABELS[stage]}
                    </Typography>
                    <Stack spacing={1}>
                      {inStage.map(c => (
                        <Paper key={c.id} variant="outlined" sx={{ p: 1, borderRadius: 2, borderColor: `${STAGE_COLORS[stage]}44` }}>
