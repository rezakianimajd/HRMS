...
const EmptyHint = ({ text }) => (
  <Paper variant="outlined" sx={{ p: 2, textAlign: 'center', borderRadius: 2.5, bgcolor: 'rgba(100,116,139,0.04)' }}>
  ...
  return (
    <Grid container spacing={2.5}>
      ...
      <Grid item xs={12} md={6}>
        <Paper sx={{ p: 2, borderRadius: 3, height: '100%', background: 'linear-gradient(135deg, rgba(245,158,11,0.08), rgba(255,255,255,0.4))' }}>
          ...
          {contracts.length === 0 ? <EmptyHint text="هیچ نسخه قراردادی ثبت نشده است" /> : (
            <Stack spacing={1}>
              {contracts.map(c => (
                <Card key={c.id} variant="outlined" sx={{ borderRadius: 2.5 }}>
      ...
      {/* اموال */}
      <Grid item xs={12} md={6}>
        <Paper sx={{ p: 2, borderRadius: 3, height: '100%', background: 'linear-gradient(135deg, rgba(16,185,129,0.08), rgba(255,255,255,0.4))' }}>
          ...
          {assets.length === 0 ? <EmptyHint text="هیچ تجهیزی به این پرسنل واگذار نشده است" /> : (
            <Stack spacing={1}>
              {assets.map(a => (
                <Card key={a.id} variant="outlined" sx={{ borderRadius: 2.5 }}>
                  <CardContent sx={{ py: 1.25, '&:last-child': { pb: 1.25 } }}>
      ...
      {/* وام‌ها */}
      <Grid item xs={12} md={6}>
        <Paper sx={{ p: 2, borderRadius: 3, height: '100%', background: 'linear-gradient(135deg, rgba(59,130,246,0.08), rgba(255,255,255,0.4))' }}>
          ...
          {loans.length === 0 ? <EmptyHint text="هیچ وامی ثبت نشده است" /> : (
            <Stack spacing={1}>
              {loans.map(l => (
                <Card key={l.id} variant="outlined" sx={{ borderRadius: 2.5 }}>
                  <CardContent sx={{ py: 1.25, '&:last-child': { pb: 1.25 } }}>
      ...
      {/* چک‌لیست ورود/خروج */}
      <Grid item xs={12} md={6}>
        <Paper sx={{ p: 2, borderRadius: 3, height: '100%', background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(255,255,255,0.4))' }}>
            ...
            <Stack spacing={1.25}>
              {checklists.map(c => (
                <Box key={c.id}>
                  ...
                  <LinearProgress variant="determinate" value={c.progress} sx={{ height: 6, borderRadius: 3, bgcolor: 'rgba(139,92,246,0.12)', '& .MuiLinearProgress-bar': { bgcolor: '#8b5cf6' } }} />
                </Box>
              ))}
            </Stack>
          )}
      ...
      {/* جرائم */}
      <Grid item xs={12} md={6}>
        <Paper sx={{ p: 2, borderRadius: 3, height: '100%', background: 'linear-gradient(135deg, rgba(239,68,68,0.08), rgba(255,255,255,0.4))' }}>
          ...
          {penalties.length === 0 ? <EmptyHint text="جریمه‌ای ثبت نشده است" /> : (
            <Stack spacing={1}>
              {penalties.map(p => (
                <Card key={p.id} variant="outlined" sx={{ borderRadius: 2.5 }}>
                  <CardContent sx={{ py: 1.25, '&:last-child': { pb: 1.25 } }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      {/* بیمه تکمیلی */}
      <Grid item xs={12} md={6}>
        <Paper sx={{ p: 2, borderRadius: 3, height: '100%', background: 'linear-gradient(135deg, rgba(14,165,233,0.08), rgba(255,255,255,0.4))' }}>
          ...
          {insurance.length === 0 ? <EmptyHint text="بیمه تکمیلی ثبت نشده است" /> : (
            <Stack spacing={1}>
              {insurance.map(i => (
                <Card key={i.id} variant="outlined" sx={{ borderRadius: 2.5 }}>
                  <CardContent sx={{ py: 1.25, '&:last-child': { pb: 1.25 } }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
